...
// v18 — Ensure globalSettings has medicalInflationRate = 0.14 (14% IRDAI benchmark)
//        and correctly patch any duplicate prepayment history keys
db.version(18).stores({}).upgrade(async tx => {
  await tx.table('globalSettings').toCollection().modify((s: any) => {
    if (s.medicalInflationRate === undefined || s.medicalInflationRate === null) {
