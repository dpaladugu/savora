...
async function fetchDashboardData(userId: string): Promise<DashboardData> {
  try {
    ...
    const monthlyExpenses = expenses
      ...
      .reduce<number>((sum: number, expense: FirestoreExpense) => {
        return sum + (typeof expense.amount === 'number' ? expense.amount : 0);
      }, 0);

    const totalExpenses = expenses.reduce<number>((sum: number, expense: FirestoreExpense) => {
      return sum + (typeof expense.amount === 'number' ? expense.amount : 0);
    }, 0);

    const totalInvestments = investments.reduce<number>((sum: number, investment: FirestoreInvestment) => {
