 ...
 * with 30-day renewal alerts, nominee tracking, and gap-analysis nudges.
 */
...
export function InsuranceManager() {
  ...
  const expiringSoon = policies.filter(p => p.endDate && daysUntil(p.endDate) <= 30 && daysUntil(p.endDate) >= 0);
  const expired      = policies.filter(p => p.endDate && daysUntil(p.endDate) < 0);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        await db.auditLogs.add({ id: crypto.randomUUID(), action: 'update', entity: 'insurancePolicy', entityId: editingId, oldValues: old, newValues: data, timestamp: new Date() });
        toast.success('Policy updated');
      } else {
        const newId = crypto.randomUUID();
        await db.insurancePolicies?.add({ id: newId, ...data });
        await db.auditLogs.add({ id: crypto.randomUUID(), action: 'create', entity: 'insurancePolicy', entityId: newId, newValues: data, timestamp: new Date() });
  ...
  const handleDelete = async (id: string) => {
    ...
    await db.auditLogs.add({ id: crypto.randomUUID(), action: 'delete', entity: 'insurancePolicy', entityId: id, oldValues: old, timestamp: new Date() });
            ...
            <Card key={p.id} className="glass">
              <CardContent className="p-4">
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                  ...
                  {p.nomineeName && <div><span className="text-muted-foreground">Nominee: </span><span className="font-medium">{p.nomineeName}</span></div>}
          ...
          <form onSubmit={handleSave} className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <Input value={form.nomineeName} onChange={e => setForm(f => ({ ...f, nomineeName: e.target.value }))} className="h-9 text-sm" placeholder="Name" />
              </div>
              <div className="space-y-1">
            ...
            <div className="flex gap-2 pt-1">
              <Button type="submit" size="sm" className="flex-1 h-9 text-xs">{editingId ? 'Update' : 'Add'}</Button>
